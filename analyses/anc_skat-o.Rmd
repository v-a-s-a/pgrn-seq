# Adjusted Neutrophil Count (ANC) SKAT-O gene-based association analysis
...
We subset our genotypes to european samples. We end up with `r nrows(eur.pheno)` samples in the following analysis.
```{r seq_skat_o, echo=FALSE}
...
```{r qqplot,
      echo=FALSE,
      caption="QQ plot for model: ANC ~ 1. Here we use only european samples."}
qqunif(basic.res$p)
```
...
```{r full_qqplot,
      echo=FALSE,
      caption="QQplot for model: ANC ~ sex + site + dose. Here we use only european samples."}
qqunif(full.res$p)
```
...
```{r pca_qqplot,
      echo=FALSE,
      caption="QQplot for model: ANC ~ sex + site + dose + PC1 + ... + PC5. Here we use all samples with non-missing phenotypes."}
qqunif(pca.res$p)
```
