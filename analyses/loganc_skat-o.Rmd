...
```{r qqplot, include=FALSE, caption="QQ plot for model: log(ANC) ~ 1. Here we use only european samples."}
qqunif(basic.res$p)
```
...
The resulting QQ plot:
```{r full_qqplot, echo=FALSE, caption="QQplot for model: log(ANC) ~ sex + site + dose. Here we use only european samples."}
qqunif(full.res$p)
```
...
The resulting QQ plot:
```{r pca_qqplot, echo=FALSE, caption="QQplot for model: log(ANC) ~ sex + site + dose + PC1 + ... + PC5. Here we use all samples with non-missing phenotypes."}
qqunif(pca.res$p)
```
