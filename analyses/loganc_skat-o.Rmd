# Log Transformed Adjusted Neutrophil Count (ANC) SKAT-O gene-based association analysis
...
eur.pheno$loganc <- log(eur.pheno$ANC)
...
```{r log_qqplot, include=FALSE, caption="QQ plot for model: log(ANC) ~ 1. Here we use only european samples."}
qqunif(basic.res$p)
```
...
full.res <- parallel.skat.scan(genes = genes,
              pheno = eur.pheno,
              model = "loganc ~ sex + site + as.numeric(dose)",
              snpMat = sparseX,
              annot = annot,
              n.core = 8)
```
...
The resulting QQ plot:
```{r log_full_qqplot, echo=FALSE, caption="QQplot for model: log(ANC) ~ sex + site + dose. Here we use only european samples."}
qqunif(full.res$p)
```
...
pheno.df$loganc <- log(pheno.df$ANC)
pca.res <- parallel.skat.scan(genes = genes,
              pheno = pheno.df,
              model = "loganc ~ sex + site + as.numeric(dose) + eigenvect.1 + eigenvect.2 + eigenvect.3 + eigenvect.4 + eigenvect.5",
              snpMat = all.sparseX,
              annot = annot,
              n.core = 8)
```
...
The resulting QQ plot:
```{r log_pca_qqplot, echo=FALSE, caption="QQplot for model: log(ANC) ~ sex + site + dose + PC1 + ... + PC5. Here we use all samples with non-missing phenotypes."}
qqunif(pca.res$p)
```
